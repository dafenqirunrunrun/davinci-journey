...
/// Repository-relative path of the archive profiles config.
const ARCHIVE_CONFIG_REL: &str = "config/archive-profiles.yml";
...
pub fn apply_publish_workspace(
    ...
    // The archive config is generated by apply_archive_profiles below, not
    // copied from the workspace, so it must NOT be passed to execute_write.
    let file_changes: Vec<RepositoryFileChange> = planned_changes
        .iter()
        .filter(|c| c.relative_path != ARCHIVE_CONFIG_REL)
        .cloned()
        .collect();
    ...
    // Write archive config (back it up first so a later failure can roll back).
    if !archive_entries.is_empty() {
        if archive_config_path.exists() {
            let backup_content = fs::read(&archive_config_path)
                .map_err(|e| format!("无法读取现有归档配置用于备份：{}", e))?;
            let backup_sha = sha256_hex(&backup_content);
            let backup_dir = repo_root
                .join(repository_transaction::TRANSACTIONS_DIR)
            ...
            let backup_path = backup_dir.join(format!(
                "{}_{}",
                backup_sha,
                ARCHIVE_CONFIG_REL.replace(['/', '\\'], "_")
            ));
            fs::write(&backup_path, &backup_content)
                .map_err(|e| format!("无法写入归档配置备份：{}", e))?;
            backups.push(repository_transaction::RepositoryBackupEntry {
                relative_path: ARCHIVE_CONFIG_REL.to_string(),
        ...
        match apply_archive_profiles(&archive_config_path, &archive_entries) {
            ...
            Err(e) => {
                // Rollback entire transaction (including the config backup)
                let mut failed_tx = transaction.clone();
                failed_tx.backups = backups;
                let _ = rollback_transaction(&repo_root, &failed_tx);
                let _ = update_transaction_status(
